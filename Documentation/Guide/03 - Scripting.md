...
This guide assumes that you have at least some basic coding knowledge and experience, preferably with C#, but not required. You should know what is a variable, a function and a loop.
...
You will still be able to use C#, and even use both C# and C++ in the same project and be able to communicate between them, although with the cost of some overhead.
...
`OnUpdate()` will start to be called the frame AFTER `OnBegin()`. Meaning that when you start a game, the first frame will call `OnBegin()` and nothing else, and the second, third, fourth, et cetera, will call `OnUpdate()`. You can call `OnUpdate()` at the end of `OnBegin()` if this behaviour is unwanted.

You can remove any of these functions if you are not using them, everything will work just fine.

NOTE: The way and order the update functions are called in is subject to change as I do not like the way this sounds in this guide lol.
...
And the output when you enter the editor and press the Play button, should look something like this.
...
As you can see, the cube is moving awfully slow, which is due to delta time being pretty small (At 60FPS, deltaTime is approximately 0.016). We could just change the code and replace `0.1f` with something larger like `2.0f`. But if  you had to change the code and rebuild the project everytime you want to change a small thing, you would go insane.
