...
Now we could multiply it by for instance `1.0f`, maybe `2.0f` and fix our problem. However I think it would be more interesting for us to have the option to control the speed from the editor, without having to rebuild our scripts.
...
When you close the editor, the value will be saved and loaded the next time you open the project.
...
now `speed` will be visible and editable in the editor, but inaccessible to everyone except `MyComponent`.

You can also use the `HideInEditor` attribute to hide a public variable in editor. Not sure why you would want that.

## Reference variables
You can also use the Copper-Engine types such as `Entity` or `Transform` as variable types. You can use this to control or get some information from another entity or transform, in your script.
...
To move another entity with this component, you can add a `Transform` variable as well.
...
It is currently invalid (nothing is assigned to it, it's equal to `null`), so you can assign it a transform by dragging any entity and dropping it on top of the field. For this I recommend you create another cube.

![image caption](Media/04/RefFields2.png)

And drag it from the Scene Hierarchy on top of the variable field.
...
- Variable: A box with a name that stores a value. Each component isntance (every entity that has your component attached to it) will store it's own version. Variables can be edited from the Copper-Editor, allowing for easier control and changes, without having to recompile or rebuild anything.